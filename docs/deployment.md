...
Do not set `PORT=8062` for Coolify or Compose host binding. `PORT` is reserved
for the app process inside the container, and moving it away from `8000` must be
matched by the compose port target and health check.
